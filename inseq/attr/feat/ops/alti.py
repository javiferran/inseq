...
class Alti(Attribution):
    ...
    def get_module(self, model, module_name, model_layer_name, layer=None):
    ...
    def attribute(
        ...
        for layer in range(n_layers):
            ln1 = self.get_module(model, config.ln1, config.layer, layer)
            dense = self.get_module(model, config.dense, config.layer, layer)
            w_o = dense.weight
            # TODO: find a better way to handle this in config yaml
            if model.config.model_type == "gpt2":
                w_o = w_o.transpose(0, 1)
            # Get W^{l,h}_V
            values_module = self.get_module(model, config.values, config.layer, layer)
